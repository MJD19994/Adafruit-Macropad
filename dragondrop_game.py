...
def background_sound(macropad, audio, filename):
    """ Start a WAV file playing in the background (non-blocking). """
    macropad.speaker_enable = True  # Enable the speaker
    audio.play(audiocore.WaveFile(open(PATH + filename, 'rb')))

def show_screen(macropad, group):
    ...
    while macropad.keys.events.get():
        pass
    while True:  # Wait for first new key press event
...
# MAIN GAME FUNCTION -------------------
def run_game(macropad):
    """Encapsulated function to run the Dragon Drop game."""
    # Ensure the display is rotated
    macropad.display.rotation = 90
    macropad.display.auto_refresh = False
    macropad.pixels.auto_write = False
    macropad.pixels.brightness = 0.5
    audio = audiopwmio.PWMAudioOut(board.SPEAKER)  # Use board.SPEAKER directly
    ...
    show_screen(macropad, title_group)  # Show title screen once
    ...
    while True:
                    ...
                    if sprite.is_fire:
                        ...
                        if y >= macropad.display.height - 40:
                            if column_pressed[column]:
                                ...
                                background_sound(macropad, audio, 'sizzle.wav')  # I smell bacon
                                sprite.paused = True
                        ...
                        if y >= macropad.display.height - 22:
                            # Egg hit ground
                            background_sound(macropad, audio, 'splat.wav')
                            sprite.paused = True
                            sprite.start_time = now
                            ...
                            if y >= macropad.display.height - 40:
                                ...
                                background_sound(macropad, audio, 'rawr.wav')
                                sprite.paused = True
                                sprite.start_time = now
                                tile.y = macropad.display.height - 22
                                tile[0] = 2  # Hatchling
        ...
        macropad.pixels.show()
        macropad.speaker_enable = False  # Disable the speaker
        for _ in sprites:
            play_group.pop(1)
        end_group[-1].text = str(score)
        show_screen(macropad, end_group)
